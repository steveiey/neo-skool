...
# favourite food.
        if "Chicken Adobo" in current_likes:
            chicken_meter += 1
print(f"{chicken_meter} people like Chicken Adobo.")
# Problem 5:
...
with open("./data_example.csv") as f:
    for line in f:
        current_likes = line.split(",")
        if "A" in current_likes[0][0]: #also dont work
            print(current_likes[0])
# does not work
# Problem 6:
# 19 people! Excellent. How many people come from Guangzhou?
with open("./data_example.csv", encoding="utf-8") as f:
    for line in f:
        Guangzhouppl = line.split(",")
        if "Guangzhou" in Guangzhouppl: #dont work
            chinameter += 1
print(f"{chinameter} people are from Guangzhou.")
